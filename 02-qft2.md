...
$$b^{s\dag}_{\mathbf{k}}\,{}_\text{in} \to
    i\int\mathrm{d}^4 x\bar\psi(x) \left(i\overleftarrow{\slashed{\partial}}+m\right)u^s({\mathbf{k}})e^{ikx}
    =\tilde{\bar\psi}(-k){-\slashed{k} - m\over i}u^s({\mathbf{k}})
    =\tilde{\bar\psi}(-k){1\over\tilde S(-k)}u^s({\mathbf{k}})$$

$$b^s_{\mathbf{k}}\,{}_\text{out} \to
    i\int\mathrm{d}^4 x e^{-ikx}\bar u^s({\mathbf{k}})\left(-i\slashed{\partial}+m\right)\psi(x)
    =\bar u^s({\mathbf{k}}){\slashed{k} - m\over i}\psi(k)
    =\bar u^s({\mathbf{k}}){1\over\tilde S(k)}\tilde\psi(k)$$

$$d^{s\dag}_{\mathbf{k}}\,{}_\text{in} \to
    -i\int\mathrm{d}^4 x e^{ikx}\bar v^s({\mathbf{k}})\left(-i\slashed{\partial}+m\right)\psi(x)
    =-\bar v^s({\mathbf{k}}){\slashed{k} - m\over i}\psi(-k)
    =-\bar v^s({\mathbf{k}}){1\over\tilde S(k)}\tilde\psi(-k)$$

$$d^s_{\mathbf{k}}\,{}_\text{out} \to
    -i\int\mathrm{d}^4 x\bar\psi(x) \left(i\overleftarrow{\slashed{\partial}}+m\right)v^s({\mathbf{k}})e^{-ikx}
    =-\tilde{\bar\psi}(k){-\slashed{k} - m\over i}v^s({\mathbf{k}})
...
$$i\slashed{\partial}\to\slashed{k}$$

$$e^{\pm ikx}\phi(x) \to \tilde\phi(\mp k)$$

$${k^2 - m^2\over i} \to {1\over\tilde D(k)}$$

$${\pm\slashed{k} - m\over i} \to {1\over\tilde S(\pm k)}$$
...
$$e^{-ip_{1'}x_{1'}}\left[\bar u^{s_{1'}}({\mathbf{k}}_{1'})
    \left(-i\slashed{\partial}_{1'} +m\right)\right]_{\alpha_{1'}}$$

$$e^{-ip_{2'}x_{2'}}\left[\bar u^{s_{2'}}({\mathbf{k}}_{2'})
    \left(-i\slashed{\partial}_{2'} +m\right)\right]_{\alpha_{2'}}$$
...
$$\left[\left(i\overleftarrow{\slashed{\partial}_1}+m\right)
    u^{s_1}({\mathbf{p}}_1)\right]_{\alpha_1} \,e^{ip_1x_1}$$

$$\left[\left(i\overleftarrow{\slashed{\partial}_2}+m\right)
...
$$\left[\bar u^{s_{1'}}({\mathbf{k}}_{1'})
    \left(-\slashed{p}_{1'} +m\right)\right]_{\alpha_{1'}}$$

$$\left[\bar u^{s_{2'}}({\mathbf{k}}_{2'})
    \left(-\slashed{p}_{2'} +m\right)\right]_{\alpha_{2'}}$$

$$\tilde G^{(4)}_{\alpha_{1'} \alpha_{2'} \alpha_1 \alpha_2}
    (p_{1'}, p_{2'}, -p_1, -p_2)$$

$$\left[\left({\slashed{p}_1}+m\right)
    u^{s_1}({\mathbf{p}}_1)\right]_{\alpha_1}$$

$$\left[\left({\slashed{p}_2}+m\right)
...
$$\tilde S(p) = {i\over \slashed{p} - m +i\epsilon}= {i(\slashed{p}+m)\over p^2-m^2+i\epsilon}$$

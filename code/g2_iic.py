"""Diagram IIc (corner): one-loop vertex part inserted at the internal
photon vertex of the LO vertex diagram (plus mirror), with the KK
"reduced diagram" renormalization: the inner vertex part is subtracted
pointwise (in its Feynman parameters) by its on-shell value
L(u,v) gamma^nu, whose integral is deltaF1(0).

String (from ubar(p') to u(p), outer photon k between vertices 4 and 1,
inner photon k2 between 5 and 3, external photon q at vertex 2):

  ubar gamma^rho S(p'-k2) gamma^nu S(p'-k-k2) gamma_rho S(p'-k)
       gamma^mu S(p-k) gamma_nu u

Inner loop (k2, Feynman parameters u for the (p'-k-k2) line, v for the
(p'-k2) line, photon rest):

  Lambda^nu(k) = P^nu(k)/D_in(k) + c (LUV - log D_in(k)) gamma^nu,
  D_in = ahat(u,v,w,lam) - bhat(u,v) k^2   (checked),

and the subtraction constant, from the same integrand at k = 0, w = 0:

  L(u,v) gamma^nu = [La(u,v)/D0 + c (LUV - log D0)] gamma^nu,
  D0 = D_in(k=0, w=0).

LUV cancels exactly in Lambda_R = Lambda - L gamma (asserted), leaving
three pieces routed through the outer loop (photon x, (p'-k) line y,
(p-k) line z):

  (a) P^nu(k)/D_in: 1/D_in = -(1/bhat) / (k^2 - CIN), CIN = ahat/bhat,
      an extra propagator (parameter t);
  (b) -La(u,v)/D0 gamma^nu: LO-type 3-denominator loop times a scalar;
  (c) -c gamma^nu log(D_in/D0): xi-representation
      log(X/Y) = int_0^1 dxi (X-Y)/(Y + xi(X-Y)),
      1/(Y + xi(X-Y)) = -(1/(xi bhat)) / (k^2 - CXI),
      CXI = (D0(1-xi) + xi ahat)/(xi bhat), numerator (ahat - D0 - bhat k^2).

CIN and CXI are kept opaque through the matrix algebra and substituted
before the w -> 0 projection. Twice the result (mirror) must reproduce
(Petermann 1957, eq. (3)):

  mu_IIc = -67/24 + pi^2/18 - zeta(3)/2 + (pi^2/3) log 2
           - (1/2) log(lam^2/m^2)
         = -0.564021... - (1/2) log(lam^2/m^2).

This script derives the parametric integrands, writes them as Fortran
includes (g2_iic_f*.inc) for numeric integration by g2_iic.f90, and
stores them in g2_iic_integrands.json for the analytic follow-up.
"""
import json
import time
...
# =================================================================
pk2 = [pp[a] - k2[a] for a in range(4)]          # p' - k2
pkk2 = [pp[a] - k[a] - k2[a] for a in range(4)]  # p' - k - k2

D_in = (u * (dot(pkk2, pkk2) - 1) + v * (dot(pk2, pk2) - 1)
...
for nu in range(4):
    Nn = zeros(4, 4)
    for rho in range(4):
        Nn += (METRIC[rho] * GAMMA[rho] * (slash(pk2) + ID4) * GAMMA[nu]
               * (slash(pkk2) + ID4) * GAMMA[rho])
...
assert resid == 0 or Poly(resid, w).monoms()[-1][0] >= 4 \
    or all(mm[0] >= 4 for mm in Poly(resid, w).monoms())
...
def sandwich_iic(mu_, sp, s_, mid_of_nu, extra_num):
    """ubar [mid^nu] S(p'-k) gamma^mu S(p-k) gamma_nu u * extra_num."""
    ub = spinor_series(ubar(breit_frame(m, w)[1], m, sp))
    uu = spinor_series(u_spinor(breit_frame(m, w)[0], m, s_))
    right_a = (slash([pp[a] - k[a] for a in range(4)]) + ID4) * GAMMA[mu_] \
        * (slash([p[a] - k[a] for a in range(4)]) + ID4)
    tot = S(0)
    for nu in range(4):
        tot += METRIC[nu] * (ub * mid_of_nu[nu] * right_a * GAMMA[nu]
                             * uu)[0, 0]
...
with open("g2_iic_integrands.json", "w") as fh:
    json.dump({nm: srepr(expr) for nm, expr in out.items()}, fh)
tick("wrote g2_iic_fa/fb/fc.inc and g2_iic_integrands.json")
print("""
mu_IIc(lam) = int fa dy dz dt du dv        (y+z+t<1, u+v<1)
            + int fb dy dz du dv           (y+z<1,   u+v<1)
            + int fc dy dz dt du dv dxi    (y+z+t<1, u+v<1)
target: -67/24 + pi^2/18 - zeta(3)/2 + pi^2 log(2)/3 - log(lam)
      = -0.564021... - log(lam)""")
